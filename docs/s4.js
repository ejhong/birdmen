(async function () {
  ...
  const SN = { hancock: "The civilisers (lost-civilisation pairs)", positive: "Documented transmissions", related: "Related by contact", random: "Random pairs (base rate)" };
  ...
  document.getElementById("verdict-p").innerHTML = `Judged blind, the civiliser pairs are not one story. Across ${h ? h.n : "–"} lineups the planted partner was ranked first <b>${h ? pc(h.share_rank1) : "–"}</b> of the time (mean rank ${h ? f2(h.mean_rank) : "–"} of 10, chance 5.5), against <b>${p ? pc(p.share_rank1) : "–"}</b> for stories with a documented common source (mean rank ${p ? f2(p.mean_rank) : "–"}), <b>${r ? pc(r.share_rank1) : "–"}</b> for pairs related by contact, and <b>${z ? pc(z.share_rank1) : "–"}</b> for random pairs. The pairs differ: ${best ? `${best[0].replace(" | ", " and ")} was picked out ${pc(best[1].share_rank1)} of the time` : ""}${worst && worst !== best ? `, ${worst[0].replace(" | ", " and ")} ${pc(worst[1].share_rank1)}` : ""}; every pair is in the table below with what beat it. The instrument works on what it should work on: ${Object.entries(BP).filter(([k, v]) => v.set === "positive").sort((a, b) => b[1].share_rank1 - a[1].share_rank1).slice(0, 3).map(([k, v]) => `${k.replace(" | ", " and ")} ${pc(v.share_rank1)}`).join(", ")}. The blinding is imperfect: Opus named the region of ${LK.region_correct} of ${LK.n} de-identified stories, so a judge may know what it is reading; that is reported per pair and is the reason to read this as a pilot.`;
  ...
  stat(`${p ? pc(p.share_rank1) : "–"}`, `Documented transmissions (Gilgamesh and Genesis, Buddha and Josaphat, Ramayana and Reamker …): the same measure.`);
  stat(`${r ? pc(r.share_rank1) : "–"} · ${z ? pc(z.share_rank1) : "–"}`, "Pairs related by contact · random pairs from different regions.");
  ...
  (function () { const box = document.getElementById("fig-sets"); const sets = ["hancock", "positive", "related", "random"].filter(s => BS[s]); const W = 1100, H = 30 + sets.length * 64; const s = svg(W, H); const L = 330; const x = v => L + v * (W - L - 300);
    ...
    box.append(s); document.getElementById("cap-sets").innerHTML = `<b>How often the planted partner was ranked first, by set and judge.</b> Dashed line: chance. Copper: the civiliser pairs. Green: the controls with a known common source or known contact. Grey: random pairs.`; })();
  // example
...
(async function () {
  ...
  const el = (tag, attrs = {}, ...kids) => { const e = document.createElement(tag); for (const [k, v] of Object.entries(attrs)) { if (k === "class") e.className = v; else if (k === "html") e.innerHTML = v; else e.setAttribute(k, v); } for (const k of kids) if (k != null) e.append(k); return e; };
  const SN = { hancock: "The civilisers", positive: "Documented transmissions", related: "Related by contact", random: "Random pairs" };
  ...
  const s = M.summary; document.getElementById("cap-mech").innerHTML = `<b>Where each pair's partner ranks among all stories, by two instruments that cannot recognise a myth.</b> Two ranks per cell: target→partner and partner→target. Median embedding rank: civiliser pairs ${s.hancock.emb_median}, documented transmissions ${s.positive.emb_median}, pairs related by contact ${s.related.emb_median}, random pairs ${s.random.emb_median} (chance is about ${Math.round(M.n_corpus / 2)}).`;
})();
