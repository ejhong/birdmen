...
- 4 Sept 2026, after the results were published and reviewed (post hoc, labelled as such on the page): a reviewer raised six objections, all accepted. (1) The prompt asks for a judgment that mixes resemblance with historical interpretation; the resemblance-only sub-score sum is now reported alongside the overall score, and a resemblance-only re-run is planned. (2) Opus and GPT-5.5 disagree about how many foreign panels outrank the moai (1 vs 61); reported prominently. (3) Same-tradition controls included pairs of photographs of the same object (Hoa Hakananai'a above all); every control pair was classified same-object / different / unsure by Claude Sonnet 5 (`s3_sameobj.py`) and the controls are reported both ways. (4) Lineup candidates were shown at about 240–250 px tall; stated, and a five-candidate re-run planned. (5) Background removal applied to the target pair only; stated, with its direction of bias. (6) Pairs of neighbouring traditions (Göbekli–Çatalhöyük, Assyria–Persepolis, Gotland–Tanum) are not a fair null; a distant null excluding them is now reported. No threshold or prediction changed.
