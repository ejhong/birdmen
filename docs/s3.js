(async function () {
  ...
  stat(`${nKept}<small> / ${nAll}</small>`, "Features on which the two unlabelled coders agreed well enough (alpha ≥ 0.67) to be used, as pre-registered. Too few to compute the pre-registered distance.");
...
(async function () {
  ...
  const verdict = `These scores come from model judgments that have not passed a perception benchmark. They do not establish whether the resemblance is ordinary or exceptional. <b>The disagreement is substantial.</b> For the whole-pillar/back pair, mean scores were ${f1(bj.opus.mean)}/10 for Opus, ${f1(bj.sonnet.mean)} for Sonnet and ${f1(bj.gpt.mean)} for GPT-5.5. Under Opus the pair was at the ${f0(pctO)}th percentile of foreign pairs${tg.opus_pct_null_distant != null ? ` and ${f0(tg.opus_pct_null_distant)}th of distant foreign pairs` : ""}; Opus rated ${nBetterO ? nBetterO.n_better : "–"} foreign panel(s) above the moai, whereas GPT-5.5 rated ${nBetterG ? nBetterG.n_better : "–"} above it. <b>Presentation also mattered.</b> In the matched monolith lineups, the moai ranked first ${mm ? `${mm.rank1} of ${mm.n}` : "–"} times; rankings differed from the earlier random-decoy condition. Matching changed object types and backgrounds while image resolution remained unequal. The preserved responses include descriptions of a face on the moai’s back and missed bird figures. These failures affect interpretation of both high and low scores. <a href="research-review.html#pictures">Read the audit</a> or <a href="recognition.html">inspect the new recognition diagnostic</a>.`;
  ...
  stat(`${f1(full.mean)}<small> / 10</small>`, `Mean similarity of Pillar 43 (whole) and the moai's back across ${full.n} unlabelled judgments by three models.`);
  stat(`${f1(nullO.mean)} · ${R.same_diffobj ? f1(R.same_diffobj.mean) : f1(same.mean)} · ${f1(pos.mean)}`, "Mean score for random foreign pairs · same-tradition pairs (different objects) · Assyria–Persepolis pairs (Opus).");
...
(async function () {
  ...
  const f2 = x => (x == null || Number.isNaN(x)) ? "–" : Number(x).toFixed(2);
  const pc = x => (x == null) ? "–" : (100 * x).toFixed(0) + "%";
  const order = ["moai_among_random", "moai_close_among_random", "moai_iso_among_random", "moai_iso_among_monoliths", "p43_among_random", "p43_iso_among_monoliths", "hard", "related_control", "same_control", "same_control_diffobj", "same_control_monoliths", "same_control_monoliths_diffobj"].filter(s => R.by_set[s]);
...
// ---------------- Qualifications (filled from data) ----------------
(async function () {
  const D = await (await fetch("data/s3.json")).json(); const R = D.judge, L = (D.lineup && D.lineup.by_set) || {}; if (!R || !R.target) return;
  const G = D.groups; const short = g => (G[g] ? G[g].label : g).split(" (")[0];
  const f1 = x => (x == null) ? "–" : Number(x).toFixed(1), f0 = x => (x == null) ? "–" : Number(x).toFixed(0), pc = x => (x == null) ? "–" : (100 * x).toFixed(0) + "%";
  const t = R.target, ro = R.rank_moai_in_p43_search_opus || {}, rg = R.rank_moai_in_p43_search_gpt || {};
  const q = { sub_pct: f0(t.opus_sub_pct_null), sub_target: f1(t.opus_sub_mean), sub_null: f1(R.null_sub_mean), n_foreign: ro.n, above_opus: ro.n_better, above_gpt: rg.n_better,
    above_gpt_groups: rg.above_by_group ? Object.entries(rg.above_by_group).map(([g, n]) => `${short(g)} ${n}`).join(", ") : "–",
    rho_gpt: R.reliability && R.reliability.gpt_on_p43_search ? R.reliability.gpt_on_p43_search.spearman.toFixed(2) : "–",
    same_n_sameobj: R.same_n_sameobj, same_n_unsure: R.same_n_unsure, same_sameobj_mean: f1(R.same_sameobj && R.same_sameobj.mean), same_all_mean: f1(R.same && R.same.mean), same_diffobj_mean: f1(R.same_diffobj && R.same_diffobj.mean), same_diffobj_pct: f0(t.opus_pct_same_diffobj),
    sc_all: pc(L.same_control && L.same_control.all && L.same_control.all.share_rank1), sc_diff: pc(L.same_control_diffobj && L.same_control_diffobj.all && L.same_control_diffobj.all.share_rank1),
    scm_all: pc(L.same_control_monoliths && L.same_control_monoliths.all && L.same_control_monoliths.all.share_rank1), scm_diff: pc(L.same_control_monoliths_diffobj && L.same_control_monoliths_diffobj.all && L.same_control_monoliths_diffobj.all.share_rank1),
    rel_mean: f1(R.null_related_pairs && R.null_related_pairs.mean), dist_mean: f1(R.null_distant && R.null_distant.mean), dist_pct: f0(t.opus_pct_null_distant), null_pct: f0(t.opus_pct_null) };
  for (const [k, v] of Object.entries(q)) for (const e of document.querySelectorAll(`[data-q="${k}"]`)) e.textContent = v == null ? "–" : v;
})();
