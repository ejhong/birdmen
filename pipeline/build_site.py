"""Assemble everything the static site needs into docs/ (data JSON + small images), the GitHub Pages root."""
...
def load_norm(s):
    if s not in norm:
        norm[s] = {r["id"]: r for r in json.load(open(os.path.join(ROOT, "data", "norm", f"{s}.json")))}
    return norm[s]
...
def main():
    results = json.load(open(os.path.join(ROOT, "data", "results", "results.json")))
    wob = json.load(open(os.path.join(ROOT, "data", "results", "wobble.json")))
    fid = json.load(open(os.path.join(ROOT, "data", "fidelity", "pairs.json")))
    ctrl = json.load(open(os.path.join(ROOT, "data", "glyphs", "control_scripts.json")))
    ...
    copy_scaled(os.path.join(ROOT, "data", "fidelity", "chart.png"), os.path.join(fig, "chart.jpg"), 1100)
    copy_scaled(os.path.join(ROOT, "data", "raw", "barthel", "tafel-005.png"), os.path.join(fig, "barthel_plate6.jpg"), 1400)
    copy_scaled(os.path.join(ROOT, "data", "raw", "mahadevan", "signlist-000.jpg"), os.path.join(fig, "mahadevan_p32.jpg"), 900)
    copy_scaled(os.path.join(ROOT, "data", "qa", "barthel_overlay_5.png"), os.path.join(fig, "barthel_overlay6.jpg"), 1400)
    copy_scaled(os.path.join(ROOT, "data", "qa", "mahadevan_overlay_0.png"), os.path.join(fig, "mahadevan_overlay32.jpg"), 900)
    copy_scaled(os.path.join(ROOT, "data", "qa", "norm_check.png"), os.path.join(fig, "norm_check.png"), 1120)
    copy_scaled(os.path.join(ROOT, "data", "qa", "wobble_check.png"), os.path.join(fig, "wobble_check.png"), 1120)
    copy_scaled(os.path.join(ROOT, "data", "qa", "top_lineara_linearb.png"), os.path.join(fig, "top_lineara_linearb.png"), 1400)
    for i in (0, 1): copy_scaled(os.path.join(ROOT, "data", "qa", f"sheet_indus_{i}.png"), os.path.join(fig, f"sheet_indus_{i}.jpg"), 1536)
    for i in range(4): copy_scaled(os.path.join(ROOT, "data", "qa", f"sheet_rongorongo_{i}.png"), os.path.join(fig, f"sheet_rongorongo_{i}.jpg"), 1536)
    ...
    site = dict(meta=meta, results={k: v for k, v in results.items() if k != "galleries"}, galleries=results["galleries"],
                ...
                calibration=json.load(open(os.path.join(ROOT, "data", "features", "calibration.json"))),
                threshold=json.load(open(os.path.join(ROOT, "data", "results", "threshold.json"))))
