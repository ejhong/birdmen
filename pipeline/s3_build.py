...
def main():
    ...
    def rec(i):
        im = imgs[i]; return dict(id=i, group=im["group"], title=im["title"], page=im["page"], license=im["license"], artist=im["artist"],
    ...
    site = dict(groups=corpus["groups"], results=strip(res), exploratory=strip(expl), jaccard=strip(jac), images={i: rec(i) for i in usable},
