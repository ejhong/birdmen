...
def run(script):
    ...
    outdir = os.path.join(ROOT, "data", "rongo", "norm", script); os.makedirs(outdir, exist_ok=True)
    recs = []
    for r in man:
    ...
    json.dump(recs, open(os.path.join(ROOT, "data", "rongo", "norm", f"{script}.json"), "w"), indent=1)
    return recs
