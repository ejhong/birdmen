...
def run(script):
    man = json.load(open(os.path.join(ROOT, "data", "rongo", "glyphs", f"{script}_manifest.json")))
    gdir = os.path.join(ROOT, "data", "rongo", "glyphs", f"{script}_w"); os.makedirs(gdir, exist_ok=True)
    out = []
    for r in man:
    ...
    json.dump(out, open(os.path.join(ROOT, "data", "rongo", "glyphs", f"{script}_w_manifest.json"), "w"), indent=1)
